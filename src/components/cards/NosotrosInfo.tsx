...
const NosotrosInfo = () => {
  ...
  return (
    <div
      className="w-full max-w-lg mx-auto p-5 sm:p-8 rounded-3xl
                 bg-white dark:bg-surface-800/80
                 border border-surface-200 dark:border-surface-700/50
                 shadow-card"
      data-aos="fade-left"
    >
      <h2 className="text-2xl sm:text-3xl font-bold text-gradient text-center mb-4 sm:mb-5">
        Quienes somos?
      </h2>

      <p className="text-surface-600 dark:text-surface-300 text-sm leading-relaxed mb-6 sm:mb-8 text-center">
      ...
      <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8">
        {features.map((f) => (
          <div key={f.title} className="flex flex-col items-center text-center group">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-gradient-to-br from-primary-500 to-accent-500
                            flex items-center justify-center mb-2 sm:mb-3
                            group-hover:scale-110 transition-transform duration-300">
              <f.icon className="text-white text-base sm:text-lg" />
            </div>
            <h3 className="text-xs sm:text-sm font-bold text-surface-800 dark:text-surface-100">{f.title}</h3>
            <p className="text-[10px] sm:text-xs text-surface-500 dark:text-surface-400 mt-1 hidden sm:block">{f.desc}</p>
          </div>
        ))}
      </div>

      <div className="flex justify-center">
        <button className="px-5 sm:px-6 py-2 sm:py-2.5 rounded-xl bg-primary-500 hover:bg-primary-600
