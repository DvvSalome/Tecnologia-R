...
const Navbar = () => {
  ...
  return (
    <nav className="flex items-center">
      ...
      <button
        ref={buttonRef}
        className="lg:hidden w-10 h-10 rounded-xl bg-surface-100/80 dark:bg-surface-800/80 flex items-center justify-center
        ...
        <div className="w-5 h-4 relative flex flex-col justify-between">
          <span
            className={`block h-0.5 rounded-full bg-surface-700 dark:bg-surface-200 transform transition-all duration-400 ease-[cubic-bezier(0.68,-0.55,0.265,1.55)] ${
              isMenuOpen ? "rotate-45 translate-y-[7px] w-full" : "w-full"
            }`}
          />
          <span
          ...
          <span
            className={`block h-0.5 rounded-full bg-surface-700 dark:bg-surface-200 transform transition-all duration-400 ease-[cubic-bezier(0.68,-0.55,0.265,1.55)] ${
              isMenuOpen ? "-rotate-45 -translate-y-[7px] w-full" : "w-1/2"
            }`}
          />
        </div>
      </button>
      ...
      <ul
        ref={menuRef}
        className={`lg:hidden absolute top-full right-0 mt-2 mr-4 w-60 glass-strong rounded-2xl shadow-lg
        ...
        {navLinks.map((link, i) => (
          ...
          >
            {link.name === "Productos" ? (
              <button
                onClick={toggleSidebar}
                className={`w-full text-left px-5 py-3.5 text-sm font-semibold transition-all duration-200
              ...
              <NavLink
                to={link.path}
                className={({ isActive }) =>
                  `block px-5 py-3.5 text-sm font-semibold transition-all duration-200
                  ${isActive
                    ? "text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-950/30"
                    : "text-surface-700 dark:text-surface-200 hover:bg-surface-100 dark:hover:bg-surface-800/50"
                  }`
      ...
      {/* Sidebar overlay */}
      <div
        className={`fixed inset-0 bg-black/50 backdrop-blur-sm z-40 transition-all duration-400
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-surface-100 dark:border-surface-800">
          <div>
            <h2 className="text-lg font-bold text-surface-900 dark:text-surface-100">Categorias</h2>
            <p className="text-xs text-surface-400 mt-0.5">Explora nuestros productos</p>
          </div>
          <button
            onClick={closeSidebar}
        ...
        <div className="p-4 overflow-y-auto h-[calc(100%-90px)]">
          {productSections.map((section, sIdx) => (
            <div
              key={section.id}
              className={`mb-3 ${isSidebarOpen ? "animate-slide-in-right" : ""}`}
            ...
            >
              ...
              >
                <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-primary-500/10 to-accent-500/10
              ...
              >
                <div className="pl-3 pr-2 py-2 space-y-0.5">
                  {section.options.map((option, oIdx) => (
                    ...
                    >
                      <span className="w-6 h-6 rounded-lg bg-surface-100 dark:bg-surface-800
